/**
 * Stateless, HMAC-signed identity handoff between this dashboard and the Moisi
 * MCP OAuth server.
 *
 * The two apps run on different origins (and in prod on different Public-Suffix
 * subdomains), so they can't share the Supabase session cookie. Instead this
 * dashboard — where the user is already signed in — mints a short-lived token
 * proving who the user is, and the MCP server verifies it with the same shared
 * secret. No DB, no network round-trip. Keep this file byte-identical to the
 * MCP copy at apps/mcp/lib/handoff.ts.
 */
